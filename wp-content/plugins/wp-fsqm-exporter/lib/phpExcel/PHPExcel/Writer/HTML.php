...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	/**
	 * Header Template for the generated HTML
	 *
	 * @var string
	 */
	protected $_htmlTemplateHeader = '';

	/**
	 * Footer Template for the generated HTML
	 *
	 * @var string
	 */
	protected $_htmlTemplateFooter = '';

	/**
	 * Additional CSS to style the output
	 * @var string
	 */
	protected $_additionalCSS = '';
	...
	/**
	 * Set the HTML template Header
	 * This template is set just after the starting body
	 *
	 * @param string $html the template html
	 */
	public function setHTMLHeader($html = '') {
		$this->_htmlTemplateHeader = $html;
	}

	/**
	 * Get the currently set HTML header template
	 *
	 * @return string the template html
	 */
	public function getHTMLHeader() {
		return $this->_htmlTemplateHeader;
	}

	/**
	 * Set the HTML template footer
	 *
	 * @param string $html the template html
	 */
	public function setHTMLFooter($html = '') {
		$this->_htmlTemplateFooter = $html;
	}

	/**
	 * Get the currently set HTML footer template
	 *
	 * @return string the template html
	 */
	public function getHTMLFooter() {
		return $this->_htmlTemplateFooter;
	}

	/**
	 * Set the additional css
	 *
	 * @param string $css the css
	 */
	public function setAdditionalCSS($css = '') {
		$this->_additionalCSS = $css;
	}

	/**
	 * Get the currently set CSS
	 *
	 * @return string the css
	 */
	public function getAdditionalCSS() {
		return $this->_additionalCSS;
	}
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		...
		$html .= $this->getHTMLHeader() . PHP_EOL;

		if($properties->getTitle()) {
			$html .= '<h1 style="text-align: center; font-size: 19pt; font-weight: bold; width: 80%; center; margin: 0 auto 10px;">' . htmlspecialchars($properties->getTitle()) . '</h1>' . PHP_EOL;
		}
		if($properties->getSubject()) {
			$html .= '<h3 style="text-align: center; font-size: 13pt; width: 80%; center; margin: 0 auto 10px; font-weight: normal;">' . htmlspecialchars($properties->getSubject()) . '</h3>' . PHP_EOL;
		}
			...
			if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
				if ($drawing->getCoordinates() == $coordinates) {
					...
					if ((!$this->_embedImages) || ($this->_isPdf)) {
						if(!$this->_isPdf) {
							$imageData = $this->getImagesRoot() . basename($filename);
						} else {
							$imageData = $filename;
						}
					} else {
					...
					//$html .= '<div style="position: relative;">';position: absolute; z-index: 1; left: ' . $drawing->getOffsetX() . 'px; top: ' . $drawing->getOffsetY() . 'px;
					$html .= '<img style=" width: ' . $drawing->getWidth() . 'px; height: ' . $drawing->getHeight() . 'px;" src="' . $imageData . '" border="0" />' . PHP_EOL;
					//$html .= '</div>';
				}
			}
		}
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		if ($generateSurroundingHTML) {
			...
			$css['html']['margin'] = '20px';
		}

		// // table { }
		// $css['table']['margin'] = '20px auto';
		// $css['table']['width'] = '80%';
		...
		// $css['table td']['padding'] = '5px';
		// $css['table td']['border'] = '1px solid #888';

		// $css['table tr.row0 td.column0']['background'] = '#888';
		// $css['table tr.row0 td.column0']['color'] = '#fff';
		// $css['table tr.row0 td.column0']['text-shadow'] = '0 0 2px #000';

		// // .gridlines td { }
		// $css['.gridlines td']['border'] = '1px solid #888';
	...
	private function _createCSSStyleAlignment(PHPExcel_Style_Alignment $pStyle) {
		...
		// Create CSS
		// $css['vertical-align'] = 'middle';// $this->_mapVAlign($pStyle->getVertical());
		// $css['padding'] = '10px';
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	public function generateHTMLFooter() {
		// Construct HTML
		$html = '';
		$html .= $this->getHTMLFooter() . PHP_EOL;
		$html .= '  </body>' . PHP_EOL;
		$html .= '</html>' . PHP_EOL;
	...
	private function _generateRow(PHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0) {
		if (is_array($pValues)) {
			...
			if (!$this->_useInlineCss) {
				...
				$html .= '		  <tr class="row' . $pRow . '" style="' . $style . '">' . PHP_EOL;
			}
			...
			foreach ($pValues as $cell) {
				...
				if ($cell instanceof PHPExcel_Cell) {
					...
					$Class .= ' style' . $cell->getXfIndex();
					$Class .= ' ' . $cell->getDataType();
